...
- ⚡ **全站增量静态再生 (ISR)**：所有核心页面（首页、Emoji、关于）均采用 ISR 模式，首屏以纯静态方式秒开，并在服务器端定期自动更新。
...
- 🚀 **极简加载逻辑**：完全移除客户端缓存依赖，利用 Next.js 原生缓存机制，大幅降低了浏览器端的计算负担。
...
- **按需加载**：目前首屏将全量 ~1000 条符号数据（约 400KB+）作为 props 传入客户端，实际仅渲染前 180 条。后续可通过 API 分页将首屏传输量减少 70%+。
...
- **骨架屏**：页面首次加载时显示骨架占位符，减少布局偏移 (CLS)。

### 搜索能力

- **服务端搜索**：当前搜索在客户端遍历全量数据，数据量增长后可将搜索逻辑上移至 API 层，支持更复杂的模糊匹配和排序。
