...
- ⚡ **全站服务端渲染 (SSR)**：所有核心页面（首页、Emoji、关于）均改为服务端组件，首屏数据由服务器直接注入 HTML，实现秒开。
- 🎲 **智能随机排列**：首页和 Emoji 页面的“全部”标签采用服务端随机打乱算法，每次访问都有新发现，且消除了客户端二次闪烁。
- 🛡️ **双端数据一致性**：通过引入 **5 分钟服务器内存缓存**，确保了 SSR 和 Hydration 阶段的数据顺序 100% 一致，彻底解决了 React 水合报错。
- 🚀 **极简加载逻辑**：移除了冗余的客户端缓存 (localStorage) 和复杂的加载状态判断，项目结构更加轻量、专注。
