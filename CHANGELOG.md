...
* **拼音搜索:** 构建数据时生成紧凑拼音索引，服务端按需读取，不再在 API 运行时加载和执行拼音库
* **搜索请求:** 增加 300 ms 防抖与中文输入法组合保护，停止输入后再请求，并关闭窗口聚焦自动刷新
* **首屏资源:** 取消本地符号字体强制预加载、顶部导航自动预取和普通用户字体诊断首包加载
* **滚动与玻璃效果:** 移除 Lenis 和实时液态折射依赖，改用原生滚动与静态 CSS 玻璃效果，降低长列表合成开销


### 问题修复

* **滚动恢复:** 在离开列表路由前保存真实滚动位置，避免原生滚动切换后返回页面位置丢失
* **分类标签:** 选中分类悬浮时保持高对比度蓝底白字，未选分类继续保留悬浮反馈
* **字体诊断:** 捕获受限存储与动态加载失败，避免低概率未处理异常
* **拼音索引:** 索引首次读取失败后允许后续请求重试，避免进程永久降级
* **Service Worker:** 安全注销旧 Worker 并清理历史字体缓存，不影响未来重新引入的根级 Worker


### 维护

* **依赖精简:** 删除 Lenis 与 simple-liquid-glass，将拼音库移至开发依赖


## [1.13.9](https://github.com/nowscott/RareCharWeb/compare/v1.13.8...v1.13.9) (2026-08-10)


### 性能优化

* **长列表:** 接入 TanStack Virtual，按动态高度网格行只渲染可视区域附近卡片，降低长时间滚动后的 DOM、布局和合成开销
* **分页加载:** 接入 TanStack Query 管理无限分页、缓存、刷新与错误状态，并在接近列表尾部前预加载下一页
* **滚动体验:** 保留服务端首屏渲染、路由返回位置和已加载分页，快速滚动与数据追加时保持列表稳定
* **卡片渲染:** 使用稳定回调与 `React.memo`，减少分页追加时已加载卡片的重复渲染


### 维护

* **依赖安全:** 升级 Next.js 及相关构建依赖，修复 npm 审计报告中的 9 个漏洞
